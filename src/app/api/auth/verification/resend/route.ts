...
export async function POST(request: Request) {
  try {
    ...
    await rateLimit(`verify:${request.headers.get("x-forwarded-for") || "local"}:${email.toLowerCase()}`, 3, 15 * 60_000);
    try {
      const user = await adminAuth().getUserByEmail(email);
      if (!user.emailVerified) await sendVerificationEmail(user.uid, email, `resend_${Math.floor(Date.now() / 900_000)}`);
